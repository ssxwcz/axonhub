...
type Config struct {
	...
	// GLM-5.2+ models that expose Zhipu's native reasoning_effort parameter. The first
	// entry whose From matches wins; values not listed pass through unchanged.
	// e.g. [{"from":"none","to":"low"}] adapts GLM-5.3 (which always thinks and only
	// accepts low/high/max) to clients that send "none".
...
// applyReasoningEffortMapping replaces the reasoning effort value according to a
// per-channel mapping. The first entry whose From matches wins; values not listed
// pass through unchanged.
func applyReasoningEffortMapping(effort string, mappings []llm.ReasoningEffortMapping) string {
	if effort == "" || len(mappings) == 0 {
		return effort
	}
	for _, mapping := range mappings {
		if mapping.From == effort {
			return mapping.To
		}
	}
	return effort
...
func resolveZaiThinking(model, reasoningEffort string, mappings []llm.ReasoningEffortMapping) (*Thinking, *string) {
	if reasoningEffort == "" {
		return nil, nil
	}

	effort := applyReasoningEffortMapping(reasoningEffort, mappings)
	...
	// GLM-5.2: thinking can still be disabled; reasoning_effort ladder is native.
...
func (t *OutboundTransformer) TransformRequest(
	...
	if llmReq.ReasoningEffort != "" {
		thinking, nativeEffort := resolveZaiThinking(llmReq.Model, llmReq.ReasoningEffort, t.ReasoningEffortMapping)
